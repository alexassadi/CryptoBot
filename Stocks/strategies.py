...
class Strategy:
    ...
    def _open_position(self, signal_result: int):
        ...
        self.trade_size = self.client.get_trade_size(self.contract, self.candles[-1].close, self.balance_pct)
        ...
        order_status = self.client.place_order(self.contract, "MARKET", self.trade_size, order_side)
        ...
        if order_status is not None:
            ...
            new_trade = Trade({"time": int(time.time() * 1000), "entry_price": avg_fill_price,
                               ...
                               "status": "open", "pnl": 0, "quantity": self.trade_size, "entry_id": order_status.order_id})
    ...
    def _check_tp_sl(self, trade: Trade):
        ...
        price = self.candles[-1].close * self.trade_size
...
class TechnicalStrategy(Strategy):
    ...
    def _rsi(self) -> [float, float]:
        ...
        return rsi.iloc[-2], rsi.iloc[-3]

    def _macd(self) -> Tuple[float, float, float, float]:
        ...
        return macd_line.iloc[-2], macd_signal.iloc[-2], macd_line.iloc[-3], macd_signal.iloc[-3]
